# -*- coding: utf-8 -*-
"""
Created on Fri Jun 14 09:59:51 2024
...
for company in companies:
    for page in pages:
        url = (
            f'http://apis.data.go.kr/B550624/fctryRegistPrdctnInfo/getFctryPrdctnService'
...
df.rename(columns={'fctryManageNo':'공장관리번호',
                   ...
                   'cmpnyTelno':'회사전화번호','cmpnyFxnum':'팩스번호',
                   'allEmplyCo':'고용인원','frstFctryRegistDe':'공장 등록일자',
                   'indutyNm':'업종명','mainProductCn':'주생산품','hmpadr':'홈페이지'
                   },inplace=True)

df.columns

df.drop(columns=['홈페이지','irsttNm'],inplace=True)

                   
df.isna().sum()

df.drop(columns=['회사전화번호','팩스번호','공장 등록일자'],inplace=True)
...
df.to_excel("지역별 업종.xlsx",index=False)
